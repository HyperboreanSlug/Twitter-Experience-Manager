 ...
 * Virtual-list safe: no display:none on cell shells; fingerprint recycle handling.
 ...
 */
    const NotifMute = {
        ...
        render() {
            ...
            pane.innerHTML = `
              <div class="tem-warn-box">
                Hides like and repost notification rows only. Uses a scroll-safe collapse
                (does not <code>display:none</code> list shells — that breaks X virtual scrolling).
              </div>
        ...
        _fingerprint(text) {
            // cheap stable-ish id of row content + filter generation
            const t = this._norm(text).slice(0, 160);
        ...
        /**
         * Build match text. Always merge a few aria-labels — X often puts
         * “X liked your post” only there, not in visible text runs.
         */
        _cellText(cell) {
            if (!cell) return '';
            const parts = [];
            try {
                // SocialContext / summary lines
                const ctx = cell.querySelector('[data-testid="socialContext"]');
                if (ctx) parts.push(ctx.innerText || ctx.textContent || '');
            } catch (_) { }
            try {
        ...
        /**
         * Collapse without display:none on the virtualized shell.
         * display:none on cellInnerDiv breaks X scroll height / recycling.
         */
        _hideCell(cell, kind) {
            if (!cell) return;
            cell.setAttribute('data-tem-like-hidden', '1');
            cell.setAttribute('data-tem-notif-kind', kind || '');
            // Collapse in-flow; keep node for virtualizer identity
            ...
            // Hide children without removing the shell from the list tree
        ...
        scanDom(force) {
            ...
            try {
                ...
                for (let i = 0; i < cells.length; i++) {
                    ...
                    if (!force && cell.getAttribute('data-tem-fp') === fp) continue;
                    ...
                    } else if (cell.getAttribute('data-tem-like-hidden') === '1') {
                        // Content changed from muted → unmuted (recycled row)
                        this._showCell(cell);
                    }
                }
